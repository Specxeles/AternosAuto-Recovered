    ...
    if selected_option == "Login" and os.path.exists(CREDENTIALS_FILE):
        with open(CREDENTIALS_FILE, "r") as f:
            ...
            if len(lines) >= 2:
                ...
                log(f"Loaded login credentials: {username}/{'*' * len(password)}")
        ...
        with Driver(uc=True, headless=False, user_data_dir=USERDATA_DIR) as driver:
            ...
            while True:
                ...
                if "https://myaccount.google.com/?utm_source=sign_in_no_continue" in current_url:
                    driver.quit()
                    messagebox.showinfo("Info", "Google account setup complete.")
                    break
                sleep(1)
    except Exception as e:
...
def set_server_number():
    ...
    messagebox.showinfo("Info", f"Server number {server_number} set")
...
options_menu.add_command(label="Login", command=lambda: set_option("Login"))
